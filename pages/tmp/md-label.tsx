...
const isDateLabel = (s: string): s is DateLabel =>
  identity<Array<string>>(dateLabels).includes(s);
...
const isPriorityLabel = (s: string): s is PriorityLabel =>
  identity<Array<string>>(priorityLabels).includes(s);
...
const isCategoryLabel = (s: string): s is CategoryLabel =>
  identity<Array<string>>(categoryLabels).includes(s);
...
const toLines = (str: string): Array<MdLine> => {
  return str.split('\n')
    .filter(l => l.length > 0)
    .map((l): MdLine => ({
      id: uuidv4(),
...
const EditModal: React.FC<{
  children?: never;
  line: MdLine | null;
  replaceLines: (oldID: string, ls: Array<MdLine>) => void;
}> = ({ line, replaceLines }) => {
...
const PageTmpMdLabel: React.FC<P> = () => {
  ...
  const replaceLines = (id: string, newLines: Array<MdLine>): void => {
      ...
      <Section title='Markdown Labeler (仮)'>
        ...
        {showMarkdownModal && (
          <Modal hider={() => setShowMarkdownModal(false)}>
            ...
            />
            <Button onClick={() =>
              window.confirm('古いデータを消しますか?') && setLines(toLines(inputText))
            }>
              置換
            </Button>
